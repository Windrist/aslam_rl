...
int main(int argc, char **argv) {
	...
	ros::param::param<float>(ns + "/range", range, 5);
	ros::param::param<std::string>(ns+"/map_topic", map_topic, "/map"); 
	...
	int i = 0;
	float xr, yr, max = 0, maxtemp = 0;
	std::vector<float> x_rand, x_nearest, x_new;
	std::vector<std::vector<float>> frontier_points;
	...
	while (ros::ok()) {
		...
		if (checking == -1) {
			...
			frontier_points.push_back(x_new);
			max = SampleRange(frontier_points);
			if (max > maxtemp) {
				range += max;
				init_map_x = range;
				init_map_y = range;
